...
public class ArcTrigger : ArcBlock
{
    ...
    {
        if (!ShouldBeCompiled) return Compiler.Compile(CompileType.Trigger, Value);
        Compiled ??= Compiler.Compile(CompileType.Trigger, Value);
        return Compiled;
    }
    public override string Compile(Block b)
    {
        return Compiler.Compile(CompileType.Trigger, b);
    }
